...
class Kohana_Gmap {
    ...
    public function address_to_ll($address) {
        ...
        $data['lat'] = $geocodedinfo->results[0]->geometry->location->lat;
        $data['lng'] = $geocodedinfo->results[0]->geometry->location->lng;
        return $data;
    }
//http://mapy.google.pl/maps?f=q&source=s_q&hl=pl&geocode=&q=zakopane+krupowki&sll=52.025459,19.204102&sspn=5.923453,19.753418&ie=UTF8&hq=&hnear=Krup%C3%B3wki,+Zakopane,+Tatrza%C5%84ski,+Ma%C5%82opolskie&ll=49.293505,19.95497&spn=0.00613,0.01929&z=16
    public function show_distance($address1, $address2){
        $geocodestring=file_get_contents('http://maps.googleapis.com/maps/api/directions/json?origin='. urlencode($address1).'&destination='. urlencode($address2).'&sensor=false');
        $geocodedinfo=json_decode($geocodestring);
        $dystans = $geocodedinfo->routes[0]->legs[0]->distance->value;
        $dystans = round($dystans/1000, 1);
        return $dystans;
    }
} // class